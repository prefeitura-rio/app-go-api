...
func TestCitizenSnapshotRepository_GetCPFsWithEnrollments(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta(`SELECT DISTINCT e.cpf FROM ( SELECT i.cpf AS cpf FROM inscricoes i UNION SELECT c.cpf AS cpf FROM emp_candidaturas c WHERE c.deleted_at IS NULL UNION SELECT cur.cpf AS cpf FROM emp_curriculos cur ) e LEFT JOIN citizen_snapshots cs ON e.cpf = cs.cpf WHERE cs.cpf IS NULL OR cs.last_synced_at < $1 LIMIT $2`)).
			WithArgs(sqlmock.AnyArg(), limit).
			WillReturnRows(rows)
